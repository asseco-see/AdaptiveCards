...
import { ActionProperty, BoolProperty, EnumProperty, GenericAction, GenericContainer, GenericInput, NumProperty, property, PropertyDefinition, SerializableObjectCollectionProperty, SerializableObjectProperty, StringProperty, Versions } from "@asseco/adaptivecards";
...
export abstract class DesignerPeerRegistry<TSource, TPeer> {  
  public elementRegistry: Adaptive.CardObjectRegistry<Adaptive.CardElement | Adaptive.Action>;
  ...
  private addProperties( definitions: any, definition: any, extensionObject: any) {
    for (const key of Object.keys(definition)) {
      ...
      if (definitionType && definitionFound) {
        const foundType = this.elementRegistry.findByName(definitionType);
        if (foundType) {
          if (typeWithoutNormalization.endsWith('[]')) {
            ...
            const decorator = property(new SerializableObjectCollectionProperty(Versions.v1_0, key, foundType.objectType));
            ...
            const decorator = property(new SerializableObjectProperty(Versions.v1_0, key, foundType.objectType));
            decorator(extensionObject.prototype, key);
          }
        }
      }
...
export class CardElementPeerRegistry extends DesignerPeerRegistry<CardElementType, CardElementPeerType> {
  reset() {
    ...
    ExtensionRegistry.subscribe(5, (schema) => {
...
export class CardDesignerSurface {
  ...
  constructor(readonly context: DesignContext) {
    ...
    CardDesignerSurface.cardElementPeerRegistry.elementRegistry = context.hostContainer.elementsRegistry;
    CardDesignerSurface.actionPeerRegistry.elementRegistry = context.hostContainer.actionsRegistry;
